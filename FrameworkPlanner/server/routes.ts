...
function isDbConnectivityError(error: any): boolean {
  ...
  if (constructorName === "ErrorEvent" || constructorName === "AggregateError" || constructorName === "TypeError") return true;

  const nested = error?.errors;
  if (Array.isArray(nested)) return nested.some(isDbConnectivityError);
  if (error?.error && typeof error.error === "object") return isDbConnectivityError(error.error);
  if (error?.cause) return isDbConnectivityError(error.cause);

  const message = String(error?.message || "");
  if (message.includes("fetch failed") || message.includes("WebSocket") || message.includes("DATABASE_URL")) return true;
