...
<<<<<<< HEAD
  ...
  async upsertTeamSettings(teamId: number, patch: Partial<InsertTeamSettings>): Promise<TeamSettings> {
    const existing = await this.getTeamSettings(teamId);
    if (existing) {
      const result = await db
        .update(teamSettings)
        .set({ ...(patch as any), updatedAt: new Date() } as any)
    ...
    const result = await db
      .insert(teamSettings)
      .values({ teamId, ...(patch as any) } as any)
      .returning();
    return result[0];
  }
...
<<<<<<< HEAD
  async getPipelineConfigByTeam(teamId: number, entityType: string): Promise<PipelineConfig | undefined> {
    const result = await db
      ...
      .where(and(eq(pipelineConfigs.teamId, teamId), eq(pipelineConfigs.entityType, entityType)))
      .limit(1);
    return result[0];
  }
  ...
  async upsertPipelineConfigByTeam(teamId: number, userId: number, entityType: string, columns: string): Promise<PipelineConfig> {
    ...
    if (existing) {
      const result = await db
        ...
        .set({ columns, userId, updatedAt: new Date() } as any)
    ...
    const result = await db
      ...
      .values({ teamId, userId, entityType, columns } as any)
      .returning();
    return result[0];
  }
